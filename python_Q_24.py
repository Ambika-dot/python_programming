"""
Q-24:-A bank has many customers and each customer has a bank account.There are also privileged customers who can earn bonus points for each of their transaction. This scenario is depicted
through the below class diagram 30 min.
Customer
- customer_id
- customer_name
- age
...
withdraw(amount): Invoke the parent class withdraw() method and increase the bonus points by calling increase_bonus() method, if no exceptions occured.If exceptions occur, display
relevant messages. Ensure that the card is taken out from the ATM under any situation.
...
c = Customer("100","Lisha",19,a)
p = PrivilegedCustomer("C101","Ambika",20,a,100)
